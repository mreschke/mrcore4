<table class='files_preview_table' border='0'><tbody><tr><td>
<?
...
foreach ($files->files as $file_class) {
    ...
    if($file_class->is_file || ($file_class->is_dir && !$files->hide_subfolders)) {
        echo "<div class='files_previewview_div'>";
            echo "<div class='files_previewview_icon'>";
                if($file_class->is_dir) { ##### ICON - DIR #####--image
                    echo "<div class='a' onclick=\"folder_click(event, false, $onclickvars);\">
                        <img src='".$file_class->icon2."' alt='icon' border='0' class='files_preview_thumbnail' />
                    </div>";
                    ...
                    echo "<div onclick=\"file_click(event, false, $onclickvars);\">";
                        if ($file_class->type == 'image' && $file_class->filename_thumb != '') {
                            $thumbclass = ($file_class->image_height > 200) ? "class='files_preview_thumbnail'" : "";
                            echo "<img src='".Page::get_url('files').'/'.$file_class->path.'/'.$file_class->filename_preview_small."' alt='preview' border='0' $thumbclass />";
                        } else {
                            echo "<img src='".$file_class->icon2."' alt='icon' border='0' class='files_preview_thumbnail' />";
                        }
                    echo "</div>";
                }
            echo "</div>";
            
            echo "<div class='files_previewview_filename'>";
